...
def prepare_and_insert_device_state(db_config, device_info, is_child=False):
    ...
    api_url = f"Connector/TPLink_SmartPlug.py {api_url_suffix}"
    ...
    cursor = connection.cursor()
    insert_query = """
    INSERT INTO device_states (device_id, state, reachable, api_url, last_updated)
    VALUES (%s, %s, %s, %s, %s);
    """
    cursor.execute(insert_query, (device_id, state_json, reachable, api_url, last_updated))
    connection.commit()
    cursor.close()
    connection.close()
...
def main():
    os.environ["DEVICE_IP"] = "192.168.88.238"
